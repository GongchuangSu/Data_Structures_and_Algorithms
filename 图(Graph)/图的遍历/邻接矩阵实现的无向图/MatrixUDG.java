/**
 * @description 邻接矩阵表示的无向图(Matrix Undirected Graph)的
...
public class MatrixUDG {
    ...
    private void BFSTraverse(int[][] mMatrix) {
        ...
        for (int i = 0; i < mVexs.length; i++) {
            ...
            while (head != rear) {                           // 若当前队列不为空
                i = queue[head++];                           // 将队中元素出队列，赋值给i
                for (int j = 0; j < mVexs.length; j++) {
                    if (mMatrix[i][j] == 1 && !visited[j]) { // 判断其他顶点是否与当前顶点存在边且未被访问过
    ...
    public static void main(String[] args) {
        ...
        matrixUDG.print();
        matrixUDG.DFSTraverse(mMatrix);
        matrixUDG.BFSTraverse(mMatrix);
    }
}